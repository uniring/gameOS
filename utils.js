...
function processButtonArt(button) {
  ...
  switch (button) {
    case "accept":
      buttonModel = screen.width == 640 ? api.keys.cancel : api.keys.accept;
      break;
    case "cancel":
      buttonModel = screen.width == 640 ? api.keys.accept : api.keys.cancel;
      break;
    case "filters":
      buttonModel = screen.width == 640 ? api.keys.details : api.keys.filters;
      break;
    case "details":
      buttonModel = screen.width == 640 ? api.keys.filters : api.keys.details;
      break;
    case "nextPage":
...
function boxArt(data) {
  if (data != null) {
    if (data.assets.boxFront.includes("/header.jpg")) return steamBoxArt(data);
    else {
      if (data.assets.boxFront != "") return data.assets.boxFront;
      else if (data.assets.poster != "") return data.assets.poster;
      else if (data.assets.banner != "") return data.assets.banner;
      else if (data.assets.tile != "") return data.assets.tile;
      else if (data.assets.cartridge != "") return data.assets.cartridge;
      else if (data.assets.logo != "") return data.assets.logo;
    }
  }
  return "";
}
...
function logo(data) {
  if (data != null) {
    if (data.assets.boxFront.includes("/header.jpg")) return steamLogo(data);
    else {
      if (data.assets.logo != "") return data.assets.logo;
    }
  }
  return "";
}
...
function fanArt(data) {
  if (data != null) {
    if (data.assets.boxFront.includes("/header.jpg")) return steamHero(data);
    else {
      if (data.assets.background != "") return data.assets.background;
      else if (data.assets.screenshots[0]) return data.assets.screenshots[0];
    }
  }
  return "";
}
